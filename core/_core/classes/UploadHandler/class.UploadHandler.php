...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'inline_file_types' => '/\.(gif|jpe?g|png|pdf|xls|xlsx|doc|docx|ppt|mp4)$/i',
            // Defines which files (based on their names) are accepted for upload:
            'accept_file_types' => '/\.(gif|jpe?g|png|pdf|xls|xlsx|doc|docx|ppt|mp4)$/i',//'/.+$/i',
		...
		if (class_exists("Imagick"))
		{
			try
			{
				if(($img_width > $max_width) or ($img_height > $max_height))
				{
					...
					if ($thumb->writeImage($new_file_path))
					{
						return true;
					}
				}
    ...
    protected function trim_file_name($name,
        $type = null, $index = null, $content_range = null)
	{
	    // odstraní cestu, whitespaces, control chars atd.
	    $name = trim(basename(stripslashes($name)), ".\x00..\x20");

	    // rozbije název na část před a po poslední tečce
	    $info = pathinfo($name);
	    $base = $info['filename'];                 // „obnažené“ jméno
	    $ext  = isset($info['extension']) ? strtolower($info['extension']) : '';

	    // proženu clear_str **pouze** base názvem (tečka zůstává pryč)
	    $base = clear_str($base, false, false);

	    // pokud po očištění nic nezbylo, použij časové razítko
	    if ($base === '') {
	        $base = str_replace('.', '-', microtime(true));
	    }

	    // pokud chybí přípona, ale z MIME typu víme, že jde o obrázek → doplníme
	    if ($ext === '' && preg_match('/^image\/(gif|jpe?g|png)/', $type, $m)) {
	        $ext = $m[1]; // gif / jpg / jpeg / png
	    }

	    // složíme zpět — přípona jen pokud nějaká existuje
