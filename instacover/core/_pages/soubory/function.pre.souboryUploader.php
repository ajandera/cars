...
{	global $C,$C_text_adresa;
    ...
    $('#fileupload_<?php echo $nameIT?>').fileupload({
        ...
        dropZone: null,
        pasteZone: null,
        acceptFileTypes: /(\.|\/)(gif|jpe?g|png|pdf|doc|docx|xls|xlsx|ppt)$/i,
			...
			$p_info = clearUnsafeChars($_POST["{$namePOST}_info"][$k]);
				...
				else
				{
					if (!empty($_FILES["{$namePOST}_aobrazek"]["name"][$k]))
					{
						...
						{
							$uudi = $edit['id'];
							$p_obrazek  =	handleUpload("{$namePOST}_aobrazek",$dirrr,false,true,$resizeWidth,$resizeHeight,$k,$thumbWidth,$thumbHeight,$cropImages,false,array('pdf','doc','docx','xls','xlsx','ppt','jpg','jpeg','png','gif'),12,array(6000,6000),75,$uudi);
							if ($p_obrazek)
							{
					...
					elseif (!empty($_POST["{$namePOST}_obrazek"][$k]))
					{
						...
						if ($madd3)
						{
							...
							$p_obrazek  =	handleMove($_POST["{$namePOST}_obrazek"][$k],$dirrr,false,true,$resizeWidth,$resizeHeight,$thumbWidth,$thumbHeight,$cropImages,false,array('pdf','doc','docx','xls','xlsx','ppt','jpg','jpeg','png','gif'),12,array(6000,6000),75,$uudi);
							if ($p_obrazek)
							{
								$edit = coreDBEdit('prirazene_obrazky',array('obrazek'=>array('value'=>$p_obrazek,'type'=>'strict')),'id = "'.$uudi.'" ');
								$pocetImgs++;
